...
function initializeBackToTop() {
  if (!backToTopBtnEl || backToTopBtnEl.dataset.bound === "true") {
    return;
  }

  const updateBackToTopVisibility = () => {
    backToTopBtnEl.classList.toggle("show", window.scrollY > 320);
  };
  ...
  backToTopBtnEl.dataset.bound = "true";
}
...
const forceEnvelopeUnlocked = true;
...
function updateEnvelopeState() {
  ...
  const isUnlocked = forceEnvelopeUnlocked || envelopeTestMode || timeLeft <= 0;

  envelopeBtnEl.disabled = !isUnlocked;
  envelopeBtnTextEl.textContent = isUnlocked ? "Open Envelope" : "Envelope Locked";

  if (isUnlocked && forceEnvelopeUnlocked) {
    proposalLockEl.textContent = "Unlocked now — open your envelope 💌";
  } else if (isUnlocked && envelopeTestMode) {
...
envelopeBtnEl.addEventListener("click", () => {
  ...
  envelopeBtnTextEl.textContent = "Envelope Opened";
...
function startYesCelebration() {
  const rainDurationMs = 9000;
  const emojiIntervalId = setInterval(dropLoveEmoji, 85);
  const wordIntervalId = setInterval(dropComplimentWord, 120);

  for (let index = 0; index < 18; index += 1) {
    setTimeout(dropLoveEmoji, index * 45);
    setTimeout(dropComplimentWord, index * 55);
  }
